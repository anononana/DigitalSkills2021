...
namespace NFCAndroidExample
{
    ...
    public class MainActivity : AppCompatActivity
    {
        ...
        protected override void OnCreate(Bundle savedInstanceState)
        {
            ...
            FindViewById<TextView>(Resource.Id.plash).Visibility = ViewStates.Visible;

            FindViewById<TextView>(Resource.Id.sum).Text = "ИТОГО:";
            FindViewById<Button>(Resource.Id.send).Text = "ПОДТВЕРДИТЬ";
            FindViewById<TextView>(Resource.Id.plash).Text = "Приложите NFC метку";

            FindViewById<Button>(Resource.Id.send).Click += (s, e) => 
            {
                var sendData = new NameValueCollection();
                sendData.Add("sum", FindViewById<EditText>(Resource.Id.sumInput).Text);
                string date = DateTime.Now.ToString("yyyy-MM-ddTHH:mm:ss");
                sendData.Add("date", date);
                sendData.Add("accessToken", Token);

                string resp = POST("http://andreifarafonow.duckdns.org:4000/receipt/create", null, sendData);
        ...
        protected override void OnNewIntent(Intent intent)
        {
            ...
            sendData.Add("nfc", int.Parse(num).ToString());

            string resp = "";

            
                resp = POST("http://andreifarafonow.duckdns.org:4000/cashier/find", null, sendData);
            ...
            try
            {
                ...
                FindViewById<TextView>(Resource.Id.plash).Visibility = ViewStates.Gone;
